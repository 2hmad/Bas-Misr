...
<html lang="ar">
  ...
  <li class="nav-item">
    <a class="nav-link active" href="edit-article.php" style="color:#333b77;font-family:'sans-serif',Cairo;padding-bottom: 16px;font-size: 17px;font-weight: 400;">تعديل موضوع</a>
  </li>
  <li class="nav-item">
...
<select type="text" name="title-article-add" style="width:100%;
    padding: 8px;
    background: #efefef;
...
<br><br>
<button type="submit" name="edit-article" style="width: 100%;
    min-height: 40px;
    ...
    color: white;
    font-weight:bold">تعديل المقال</button>
...
if(isset($_POST['edit-article'])){
  ...
  if (!empty($_POST['title-article-add']) && !empty($_FILES['file-article']['name']) && !empty($_POST['article-body'])) {
    $sql = "UPDATE articles SET writer = '".$article_writer."', cover = '".$article_image."', body = '".$article_body."', section = '".$article_section."', date = '".$article_date."' WHERE title = '".$title."'";
    $query = mysqli_query($connect, $sql);
    echo "<div class='alert alert-success' role='alert'>تم تعديل المقال بنجاح</div>";  
  } else {
    echo "<div class='alert alert-danger' role='alert'>لم يتم تعديل المقال</div>";
}
}
?>
    </form>
